...
public class Swagger2Config {
    ...
    public Docket createRestApi() {
    	return new Docket(DocumentationType.SWAGGER_2)   // 初始化swagger2 api配置
    	        ...
    	        .apis(RequestHandlerSelectors.withClassAnnotation(Controller.class))    // api筛选模式，可选值有any / none / withClassAnnotation / withMethodAnnotation / basePackage
    	        ...
    	        .globalResponseMessage(RequestMethod.GET,    // 重写response message
    	            newArrayList(new ResponseMessageBuilder()
    	                ...
    	                .build()))
				/*
				 * .securitySchemes(newArrayList(apiKey())) // 提供的securtiy schemas有 ApiKey,
				 * BasicAuth, OAuth .securityContexts(newArrayList(securityContext())) // ???
				 */
...
//    	                .build()))
	 ...
	 * authorizationScope; return newArrayList( new SecurityReference("mykey",
	 * authorizationScopes)); }
	 * 
	 * @Bean SecurityConfiguration security() { return
	 * SecurityConfigurationBuilder.builder() .clientId("test-app-client-id")
	 * .clientSecret("test-app-client-secret") .realm("test-app-realm")
